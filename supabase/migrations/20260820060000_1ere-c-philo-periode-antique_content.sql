...
-- Moodle course id 3735: "Leçon 6_Présentation de l'histoire de la
-- philosophie" (https://lyc.ecole-ci.org/course/view.php?id=3735),
-- resource id 81810 (Philo_1C_lecon6). Contenu réécrit à partir du PDF source
-- (programme officiel MENET-FP, thème "aperçu de l'histoire de la
-- philosophie", leçon sur la période antique — PDF commun "toutes séries").
-- Faits historiques et doctrinaux réels (dates, biographies, doctrines des
-- philosophes antiques) et courtes citations de philosophes antiques du
-- domaine public (Héraclite, Parménide, Platon, Épictète) utilisées à des
-- fins d'illustration pédagogique ; synthèses reformulées, non copiées
-- verbatim.
insert into public.courses (
  ...
  jsonb_build_object(
    'situation', jsonb_build_object(
      'text', $$Le chef de la classe de 1ère A2 du lycée moderne 3 de Gagnoa, surnommé « Platon », affirme que Platon est un philosophe moderne. Ses camarades le contredisent, affirmant qu'il appartient à la période antique. Pour trancher, la classe entreprend des recherches sur les principaux auteurs de la période antique et leurs conceptions philosophiques.$$
    ),
